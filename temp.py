...
def do_training(args, update_op, loss,hooks):
    ...
    with tf.train.MonitoredTrainingSession(checkpoint_dir=checkpoint_dir,
        ...
        while not sess.should_stop():
            ...
            # if step % 20 == 0:
                ...
                # summary_str = sess.run(summary,options=run_options,run_metadata=run_metadata)
...
def create_parallel_optimization(args,model_fn, input_fn, optimizer, controller="/cpu:0"):
    ...
    with tf.variable_scope(tf.get_variable_scope()) as outer_scope:
        ...
        # Retain the summaries from the final tower.
        # summaries = tf.get_collection(tf.GraphKeys.SUMMARIES, outer_scope)
        ...
        hooks = [
            ...
            tf.train.StopAtStepHook(last_step=args["max_steps"]),
...
def parallel_training(args,model_fn, dataset):
    ...
    optimizer = tf.train.RMSPropOptimizer(learning_rate=args["learning_rate"] * hvd.size())

    optimizer = hvd.DistributedOptimizer(optimizer)

    #optimizer = tf.train.AdamOptimizer(learning_rate=args["learning_rate"])
    update_op, loss, hooks = create_parallel_optimization(args,
                                                            model_fn,
                                                            input_fn,
                                                            optimizer)

    do_training(args,update_op, loss, hooks)
